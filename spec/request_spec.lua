...
describe("Request", function()
  ...
  describe("chunk encoding", function()
    ...
    describe("inbound", function()
      ...
      it("get_body buffers partials", function()
        local r = assert(Request.source(test_utils.create_timeing_out_source(
          test_utils.wikipedia_chunks.request)))
        local b, err = r:get_body()
        assert(not b, "unexpected successful recieve on timeout")
        assert(err == "timeout", string.format("error expected err``%s` to be `timeout`", err))
        b = assert(r:get_body())
        assert.are.equal(test_utils.wikipedia_chunks.assert_body, b or nil)
      end)

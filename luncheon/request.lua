...
local Headers = require 'luncheon.headers'.Headers
...
local function parse_preamble(line)
    ...
    assert(start, string.format('Invalid http request first line: "%s"', line))
    return {
        method = method,
...
function Request.new(socket)
    assert(socket, 'cannot create request with nil socket')
    local r = {
        socket = socket,
